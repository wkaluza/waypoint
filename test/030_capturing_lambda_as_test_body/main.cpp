...
WAYPOINT_AUTORUN(waypoint::TestRun const &t)
{
  ...
  t.test(g1, "Basic capture")
    .run(
      [captured0, captured1, captured2, captured3](waypoint::Context const &ctx)
      {
  ...
  t.test(g1, "Movable & non-copyable capture")
    .run(
      [captured4 = std::move(captured4)](waypoint::Context const &ctx)
      {
  ...
  t.test(g1, "Copyable & non-movable capture")
    .run(
      [captured5](waypoint::Context const &ctx)
      {
