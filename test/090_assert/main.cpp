...
WAYPOINT_AUTORUN(waypoint::TestRun const &t)
{
  ...
  t.test(g1, "Test 1")
    .run(
      ...
      {
        ctx.assert(true);
        waypoint::internal::assert(true, "assertion passed");
      });
  ...
  t.test(g1, "Test 2")
    .run(
      ...
      {
        ctx.assert(true);
        waypoint::internal::assert(false, "assertion failed");
      });
}
...
auto main() -> int
{
  ...
  REQUIRE_IN_MAIN(
    test_count == 2,
    std::format("Expected test_count to be 2, but it is {}", test_count));

  std::vector const expected_statuses = {
    waypoint::TestOutcome::Status::Success,
    waypoint::TestOutcome::Status::Terminated};
  std::vector const expected_disabled_states = {false, false};
  std::vector const expected_assertion_counts = {1U, 1U};
